...
function BrandBadge({ brand }: { brand: string }) {
  ...
  return (
    <span
      className={`px-2 py-1 text-xs font-medium rounded-lg border ${getBrandColor(
        brand
...
export function PaymentMethodList({
  ...
  const [creatingIntent, setCreatingIntent] = React.useState<string | null>(
    null
  );

  const handleCreatePurchaseIntent = async (paymentMethod: any) => {
    setCreatingIntent(paymentMethod.id);
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 ml-4">
              <button
                ...
                disabled={creatingIntent === method.id}
