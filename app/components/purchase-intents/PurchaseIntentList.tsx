...
function BrandBadge({ brand, last4 }: { brand?: string; last4?: string }) {
  ...
  return (
    <span
      className={`px-2 py-1 text-xs font-medium rounded-lg border ${getBrandStyle(
        brand
...
export function PurchaseIntentList({
  ...
  const getPaymentMethodForIntent = (
    ...
    return (
      paymentMethods.find((pm) => pm.id === intent.paymentMethodId) || null
    );
  };
  ...
  const handleGetCard = async (intent: PurchaseIntent) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/purchase-intents/${intent.id}/details`,
        { headers }
      );
      ...
      if (cardBrand === "mastercard" && data.card) {
        ...
        throw new Error(
          `No card details available for this ${cardBrand} purchase intent`
        );
      }
    } catch (error) {
  ...
  const handleGetToken = async (intent: PurchaseIntent) => {
    ...
    if (
      !cardBrand ||
      !["amex", "american-express", "discover"].includes(cardBrand)
    ) {
    ...
    if (intent.credentialType !== "network-token") {
      onError?.(
        "Network token is only available for network-token credential type"
      );
      return;
    }
    ...
    try {
      ...
      if (data.token || data.tokenData || data.networkToken) {
        ...
        throw new Error(
          `No network token data available for this ${cardBrand} purchase intent`
        );
      }
    } catch (error) {
            ...
            <div className="flex items-center gap-2 ml-4">
              ...
              {(() => {
                ...
                const showGetCardButton =
                  cardBrand &&
                  ["mastercard", "visa"].includes(cardBrand) &&
                  intent.status === "active";
                const showGetTokenButton =
                  cardBrand &&
                  ["amex", "american-express", "discover"].includes(
                    cardBrand
                  ) &&
